...
def excel_file():
    ...
    # Add the '+3 Sigma' & '-3 Sigma' from the latest file in the '3Sigma' folder
    df = df.merge(df_sigma[['Limit Name','+3 Sigma','-3 Sigma']],on='Limit Name')
    df.drop_duplicates(subset=['Limit Name'], keep='first', inplace=True, ignore_index=True)
...
cnt = 1
lastpos = 2
keyword_list = ['rev check','Check_fans','Check_temp','V_prog_acc','V_rdbk_acc','Cc_rdbk_acc_lo:',\
    'Cc_prog_acc','Cc_rdbk_acc:','V_load_reg','V_line_reg','Trans_resp','Ovp_acc','CV_prg_up','CV_prg_dn',\
    'Cv_prog_resp','Cc_load_reg','Cc_line_reg','_noise','Cmi_test']

lastpos, cnt = create_subtable_keyword(df,"rev check",writer,'Rev Check',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keywords(df,["Check_fans","Check_temp"],writer,'Check_fans , Check_temp',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"V_prog_acc",writer,'V Prog Accuracy',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"V_rdbk_acc",writer,'V Rdbk Accuracy',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"Cc_rdbk_acc_lo:",writer,'Cc_rdbk_acc_lo',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"Cc_prog_acc",writer,'Cc_prog_acc',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"Cc_rdbk_acc:",writer,'Cc_rdbk_acc',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keywords(df,["V_load_reg","V_line_reg"],writer,'Voltage Load\Line',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"Trans_resp",writer,'Transient Response',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"Ovp_acc",writer,'OVP Accuracy',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"CV_prg_up",writer,'CV_prg_up',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"CV_prg_dn",writer,'CV_prg_dn',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"Cv_prog_resp",writer,'Cv_prog_resp',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keywords(df,["Cc_load_reg","Cc_line_reg"],writer,'Cc Load\Line',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"_noise",writer,'Noise Test',startrowindex=cnt,lastpos=lastpos)
lastpos, cnt = create_subtable_keyword(df,"Cmi_test",writer,'Cmi_test',startrowindex=cnt,lastpos=lastpos)
